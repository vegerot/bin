...
def main():
    ...
    charging=re.compile(r'AC Charger Information:.*?Connected: (?P<connected>(Yes|No))(.*?Wattage \(W\): (?P<wattage>(\d+)))?\n.*?Charging: (?P<charging>(Yes|No))',re.DOTALL)
    ...
    try:
        ...
        timeRemaining='Not Charging' if (parsedInfo.group('charging')=='No' and parsedInfo.group('connected')=='Yes') else "No Estimate"
        ret=1
